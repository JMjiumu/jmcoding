<template><div><h1 id="springcloud02" tabindex="-1"><a class="header-anchor" href="#springcloud02" aria-hidden="true">#</a> SpringCloud02</h1>
<blockquote>
<p>by九牧</p>
</blockquote>
...
<blockquote>
<p><code v-pre>Nacos</code> 除了可以做注册中心，同样可以做配置管理来使用</p>
</blockquote>
...
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714164426792.png" alt="image-20210714164426792"></p>
<blockquote>
<p><code v-pre>Nacos</code> 一方面可以将配置集中管理，另一方可以在配置变更时，及时通知微服务，实现配置的热更新</p>
</blockquote>
...
<p>如何在 <code v-pre>nacos</code> 中管理配置呢？</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714164742924.png" alt="image-20210714164742924"></p>
<p>然后在弹出的表单中，填写配置信息：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714164856664.png" alt="image-20210714164856664"></p>
<blockquote>
<p><strong>注意</strong>：项目的核心配置，需要热更新的配置才有放到 <code v-pre>nacos</code> 管理的必要，基本不会变更的一些配置还是保存在微服务本地比较好</p>
</blockquote>
<h3 id="_1-1-2-从微服务拉取配置" tabindex="-1"><a class="header-anchor" href="#_1-1-2-从微服务拉取配置" aria-hidden="true">#</a> 1.1.2 从微服务拉取配置</h3>
<blockquote>
<p>微服务要拉取 <code v-pre>nacos</code> 中管理的配置，并且与本地的 <code v-pre>application.yml</code> 配置合并，才能完成项目启动。但如果尚未读取 <code v-pre>application.yml</code>，又如何得知 <code v-pre>nacos</code> 地址呢？</p>
<p>因此 <code v-pre>Spring</code> 引入了一种新的配置文件：<code v-pre>bootstrap.yaml</code> 文件，会在 <code v-pre>application.yml</code> 之前被读取，流程如下：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/L0iFYNF.png" alt="img"></p>
</blockquote>
<h4 id="_1、引入nacos-config依赖" tabindex="-1"><a class="header-anchor" href="#_1、引入nacos-config依赖" aria-hidden="true">#</a> 1、引入nacos-config依赖</h4>
<p>首先，在 <code v-pre>user-service</code> 服务中，引入 <code v-pre>nacos-config</code> 的客户端依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!--nacos配置管理依赖--></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2、添加bootstrap-yaml" tabindex="-1"><a class="header-anchor" href="#_2、添加bootstrap-yaml" aria-hidden="true">#</a> 2、添加bootstrap.yaml</h4>
<p>然后，在 <code v-pre>user-service</code> 中添加一个 <code v-pre>bootstrap.yaml</code> 文件，内容如下：</p>
...
<p>这里会根据 <code v-pre>spring.cloud.nacos.server-addr</code> 获取 <code v-pre>nacos</code> 地址，再根据<code v-pre>${spring.application.name}-${spring.profiles.active}.${spring.cloud.nacos.config.file-extension}</code> 作为文件id，来读取配置</p>
<p><strong>注意</strong>：最好是删除或者注释掉 <code v-pre>application.yml</code> 配置文件中的相同内容</p>
</blockquote>
<p>本例中，就是去读取 <code v-pre>userservice-dev.yaml</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714170845901.png" alt="image-20210714170845901"></p>
<h4 id="_3、读取nacos配置" tabindex="-1"><a class="header-anchor" href="#_3、读取nacos配置" aria-hidden="true">#</a> 3、读取nacos配置</h4>
<p>在 <code v-pre>user-service</code> 中的 <code v-pre>UserController</code> 中添加业务逻辑，读取 <code v-pre>pattern.dateformat</code> 配置：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123204025921.png" alt="image-20230123204025921"></p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 完整代码</span>
...
<p>在页面访问，可以看到效果：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123204116607.png" alt="image-20230123204116607"></p>
</blockquote>
<h2 id="_1-2-配置热更新" tabindex="-1"><a class="header-anchor" href="#_1-2-配置热更新" aria-hidden="true">#</a> 1.2 配置热更新</h2>
<blockquote>
<p>我们最终的目的，是修改 <code v-pre>nacos</code> 中的配置后，微服务中无需重启即可让配置生效，也就是<strong>配置热更新</strong>，要实现配置热更新，可以使用两种方式</p>
</blockquote>
<h3 id="_1-2-1-方式一" tabindex="-1"><a class="header-anchor" href="#_1-2-1-方式一" aria-hidden="true">#</a> 1.2.1 方式一</h3>
<p>在 <code v-pre>@Value</code> 注入的变量所在类上添加注解 <code v-pre>@RefreshScope</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714171036335.png" alt="image-20210714171036335"></p>
<h3 id="_1-2-2-方式二" tabindex="-1"><a class="header-anchor" href="#_1-2-2-方式二" aria-hidden="true">#</a> 1.2.2 方式二</h3>
<p>使用 <code v-pre>@ConfigurationProperties</code> 注解代替 <code v-pre>@Value</code> 注解。</p>
<p>在 <code v-pre>user-service</code> 服务中，添加一个类，读取 <code v-pre>patterrn.dateformat</code> 属性：</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PatternProperties</span> <span class="token punctuation">{</span>
    <span class="token keyword">private</span> <span class="token class-name">String</span> dateformat<span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 <code v-pre>UserController</code> 中使用这个类代替 <code v-pre>@Value</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714171316124.png" alt="image-20210714171316124"></p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 完整代码</span>
<span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>jmcoding<span class="token punctuation">.</span>user<span class="token punctuation">.</span>web</span><span class="token punctuation">;</span>
...
<blockquote>
<p>其实微服务启动时，会去 <code v-pre>nacos</code> 读取多个配置文件，例如：</p>
<ul>
<li><code v-pre>[spring.application.name]-[spring.profiles.active].yaml</code>，如：<code v-pre>userservice-dev.yaml</code></li>
<li><code v-pre>[spring.application.name].yaml</code>，如：<code v-pre>userservice.yaml</code></li>
</ul>
<p>而<code v-pre>[spring.application.name].yaml</code>不包含环境，因此可以被多个环境共享（下面我们通过案例来测试配置共享）</p>
</blockquote>
<h3 id="_1、添加一个环境共享配置" tabindex="-1"><a class="header-anchor" href="#_1、添加一个环境共享配置" aria-hidden="true">#</a> 1、添加一个环境共享配置</h3>
<p>我们在 <code v-pre>nacos</code> 中添加一个 <code v-pre>userservice.yaml</code> 文件：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714173233650.png" alt="image-20210714173233650"></p>
<blockquote>
<p>上面的 <code v-pre>envSharedValue</code> 的值没什么特殊含义，就是为了测试</p>
</blockquote>
...
<p>在 <code v-pre>user-service</code> 服务中，修改 <code v-pre>PatternProperties</code> 类，读取新添加的属性：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714173324231.png" alt="image-20210714173324231"></p>
<p>在 <code v-pre>user-service</code> 服务中，修改 <code v-pre>UserController</code>，添加一个方法：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123211159895.png" alt="image-20230123211159895"></p>
...
<p>修改 <code v-pre>UserApplication2</code> 这个启动项，改变其 <code v-pre>profile</code> 值：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714173538538.png" alt="image-20210714173538538"></p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714173519963.png" alt="image-20210714173519963"></p>
<blockquote>
<p>这样，<code v-pre>UserApplication(8081)</code> 使用的 <code v-pre>profile</code> 是 <code v-pre>dev</code>，<code v-pre>UserApplication2(8082)</code> 使用的 <code v-pre>profile</code> 是<code v-pre>test</code></p>
<p><strong>启动前</strong>记得把 <code v-pre>UserController</code> 中的 <code v-pre>dateformat</code> 进行注释，因为使用 <code v-pre>UserApplication2(8082)</code> 进行访问时会报错（找不到 <code v-pre>dateformat</code> 的值）</p>
</blockquote>
<p>启动 <code v-pre>UserApplication</code> 和 <code v-pre>UserApplication2</code>，访问<a href="http://localhost:8081/user/prop%EF%BC%8C%E7%BB%93%E6%9E%9C%EF%BC%9A" target="_blank" rel="noopener noreferrer">http://localhost:8081/user/prop，结果：<ExternalLinkIcon/></a></p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123211607466.png" alt="image-20230123211607466"></p>
<p>访问<a href="http://localhost:8082/user/prop%EF%BC%8C%E7%BB%93%E6%9E%9C%EF%BC%9A" target="_blank" rel="noopener noreferrer">http://localhost:8082/user/prop，结果：<ExternalLinkIcon/></a></p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123211617922.png" alt="image-20230123211617922"></p>
<p>可以看出来，不管是 <code v-pre>dev</code>，还是 <code v-pre>test</code> 环境，都读取到了 <code v-pre>envSharedValue</code> 这个属性的值。</p>
<h3 id="_4、配置共享的优先级" tabindex="-1"><a class="header-anchor" href="#_4、配置共享的优先级" aria-hidden="true">#</a> 4、配置共享的优先级</h3>
<p>当 <code v-pre>nacos</code>、服务本地同时出现相同属性时，优先级有高低之分：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714174623557.png" alt="image-20210714174623557"></p>
<h2 id="_1-4-搭建nacos集群" tabindex="-1"><a class="header-anchor" href="#_1-4-搭建nacos集群" aria-hidden="true">#</a> 1.4 搭建Nacos集群</h2>
<p><code v-pre>Nacos</code> 生产环境下一定要部署为集群状态，部署方式参考课前资料中的文档：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714174728042.png" alt="image-20210714174728042"></p>
<h1 id="_2-feign远程调用" tabindex="-1"><a class="header-anchor" href="#_2-feign远程调用" aria-hidden="true">#</a> 2. Feign远程调用</h1>
<p>先来看我们以前利用 <code v-pre>RestTemplate</code> 发起远程调用的代码：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714174814204.png" alt="image-20210714174814204"></p>
<p>存在下面的问题：</p>
<ul>
<li>
<p>代码可读性差，编程体验不统一</p>
...
<li>
<p>参数复杂 <code v-pre>URL</code> 难以维护</p>
</li>
</ul>
<blockquote>
<p>而<code v-pre>Feign</code> 是一个声明式的 <code v-pre>http</code> 客户端，官方地址：<a href="https://github.com/OpenFeign/feign%EF%BC%8C%E5%85%B6%E4%BD%9C%E7%94%A8%E5%B0%B1%E6%98%AF%E5%B8%AE%E5%8A%A9%E6%88%91%E4%BB%AC%E4%BC%98%E9%9B%85%E7%9A%84%E5%AE%9E%E7%8E%B0" target="_blank" rel="noopener noreferrer">https://github.com/OpenFeign/feign，其作用就是帮助我们优雅的实现<ExternalLinkIcon/></a> <code v-pre>http</code> 请求的发送，解决上面提到的问题。</p>
</blockquote>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714174918088.png" alt="image-20210714174918088"></p>
...
<blockquote>
<p><code v-pre>Fegin</code> 的使用步骤如下</p>
</blockquote>
<h3 id="_1、引入依赖" tabindex="-1"><a class="header-anchor" href="#_1、引入依赖" aria-hidden="true">#</a> 1、引入依赖</h3>
<p>我们在 <code v-pre>order-service</code> 服务的 <code v-pre>pom</code> 文件中引入 <code v-pre>feign</code> 的依赖：</p>
...
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123213657520.png" alt="image-20230123213657520"></p>
<h3 id="_3、编写feign的客户端" tabindex="-1"><a class="header-anchor" href="#_3、编写feign的客户端" aria-hidden="true">#</a> 3、编写Feign的客户端</h3>
<p>在 <code v-pre>order-service</code> 中新建一个接口，内容如下：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个客户端主要是基于 <code v-pre>SpringMVC</code> 的注解来声明远程调用的信息，比如：</p>
<ul>
...
<p>这样，<code v-pre>Feign</code> 就可以帮助我们发送 <code v-pre>http</code> 请求，无需自己使用 <code v-pre>RestTemplate</code> 来发送了。</p>
<h3 id="_4、测试" tabindex="-1"><a class="header-anchor" href="#_4、测试" aria-hidden="true">#</a> 4、测试</h3>
<p>修改 <code v-pre>order-service</code> 中的 <code v-pre>OrderService</code> 类中的 <code v-pre>queryOrderById</code> 方法，使用 <code v-pre>Feign</code> 客户端代替 <code v-pre>RestTemplate</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20230123214611447.png" alt="image-20230123214611447"></p>
<blockquote>
<p>是不是看起来优雅多了🤩</p>
</blockquote>
<h3 id="_5、总结" tabindex="-1"><a class="header-anchor" href="#_5、总结" aria-hidden="true">#</a> 5、总结</h3>
<blockquote>
<p>使用 <code v-pre>Feign</code> 的步骤：</p>
<p>① 引入依赖</p>
<p>② 添加 <code v-pre>@EnableFeignClients</code> 注解</p>
<p>③ 编写 <code v-pre>FeignClient</code> 接口</p>
<p>④ 使用 <code v-pre>FeignClient</code> 中定义的方法代替 <code v-pre>RestTemplate</code></p>
</blockquote>
<h2 id="_2-2-自定义配置" tabindex="-1"><a class="header-anchor" href="#_2-2-自定义配置" aria-hidden="true">#</a> 2.2 自定义配置</h2>
<p><code v-pre>Feign</code> 可以支持很多的自定义配置，如下表所示：</p>
<table>
<thead>
<tr>
<th>类型</th>
...
<blockquote>
<p>一般情况下，默认值就能满足我们使用，如果要自定义时，只需要创建自定义的 <code v-pre>@Bean</code> 覆盖默认 <code v-pre>Bean</code> 即可</p>
</blockquote>
...
<p>基于配置文件修改 <code v-pre>feign</code> 的日志级别可以针对单个服务：</p>
...
<ul>
<li><code v-pre>NONE</code>：不记录任何日志信息，这是默认值。</li>
<li><code v-pre>BASIC</code>：仅记录请求的方法，<code v-pre>URL</code> 以及响应状态码和执行时间</li>
<li><code v-pre>HEADERS</code>：在 <code v-pre>BASIC</code> 的基础上，额外记录了请求和响应的头信息</li>
<li><code v-pre>FULL</code>：记录所有请求和响应的明细，包括头信息、请求体、元数据。</li>
</ul>
<h3 id="_2-2-2-java代码方式" tabindex="-1"><a class="header-anchor" href="#_2-2-2-java代码方式" aria-hidden="true">#</a> 2.2.2 Java代码方式</h3>
<p>也可以基于 <code v-pre>Java</code> 代码来修改日志级别，先声明一个类，然后声明一个 <code v-pre>Logger.Level</code> 的对象：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DefaultFeignConfiguration</span>  <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果要<strong>全局生效</strong>，将其放到启动类的 <code v-pre>@EnableFeignClients</code> 这个注解中：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果是<strong>局部生效</strong>，则把它放到对应的 <code v-pre>@FeignClient</code> 这个注解中：</p>
...
<blockquote>
<p><code v-pre>Feign</code> 底层发起 <code v-pre>http</code> 请求，依赖于其它的框架，其底层客户端实现包括：</p>
<ul>
<li><code v-pre>URLConnection</code>：默认实现，不支持连接池</li>
<li><code v-pre>Apache HttpClient</code> ：支持连接池</li>
<li><code v-pre>OKHttp</code>：支持连接池</li>
</ul>
</blockquote>
<p>因此提高 <code v-pre>Feign</code> 的性能主要手段就是使用<strong>连接池</strong>代替默认的 <code v-pre>URLConnection</code>， 这里我们用 <code v-pre>Apache</code> 的<code v-pre>HttpClient</code> 来演示。</p>
<h4 id="_1、引入依赖-1" tabindex="-1"><a class="header-anchor" href="#_1、引入依赖-1" aria-hidden="true">#</a> 1、引入依赖</h4>
<p>在 <code v-pre>order-service</code> 的 <code v-pre>pom</code> 文件中引入 <code v-pre>Apache</code> 的 <code v-pre>HttpClient</code> 依赖：</p>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2、配置连接池" tabindex="-1"><a class="header-anchor" href="#_2、配置连接池" aria-hidden="true">#</a> 2、配置连接池</h4>
<p>在 <code v-pre>order-service</code> 的 <code v-pre>application.yml</code> 中添加配置：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">feign</span><span class="token punctuation">:</span>
  <span class="token key atrule">client</span><span class="token punctuation">:</span>
    <span class="token key atrule">config</span><span class="token punctuation">:</span>
...
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714185925910.png" alt="image-20210714185925910"></p>
<p><code v-pre>Debug</code> 方式启动 <code v-pre>order-service</code> 服务，可以看到这里的 <code v-pre>client</code>，底层就是 <code v-pre>Apache HttpClient</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714190041542.png" alt="image-20210714190041542"></p>
<h4 id="_3、总结" tabindex="-1"><a class="header-anchor" href="#_3、总结" aria-hidden="true">#</a> 3、总结</h4>
<p><code v-pre>Feign</code> 的优化：</p>
<ol>
<li>
<p>日志级别尽量用 <code v-pre>basic</code></p>
</li>
<li>
<p>使用 <code v-pre>HttpClient</code> 或 <code v-pre>OKHttp</code> 代替 <code v-pre>URLConnection</code></p>
<ol>
...
<blockquote>
<p>所谓最佳实践，就是使用过程中总结的经验，最好的一种使用方式。自习观察可以发现，<code v-pre>Feign</code> 的客户端与服务提供者的 <code v-pre>controller</code> 代码非常相似</p>
</blockquote>
<p><code v-pre>feign</code>客户端：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714190542730.png" alt="image-20210714190542730"></p>
<p><code v-pre>UserController</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714190528450.png" alt="image-20210714190528450"></p>
...
<ol>
<li>
<p>定义一个 <code v-pre>API</code> 接口，利用定义方法，并基于 <code v-pre>SpringMVC</code> 注解做声明</p>
</li>
<li>
<p><code v-pre>Feign</code> 客户端和 <code v-pre>Controller</code> 都集成改接口</p>
</li>
</ol>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714190640857.png" alt="image-20210714190640857"></p>
<p>优点：</p>
<ul>
<li>简单</li>
<li>实现了代码共享</li>
...
<li>
<p>参数列表中的注解映射并不会继承，因此 <code v-pre>Controller</code> 中必须再次声明方法、参数列表、注解</p>
</li>
</ul>
<h3 id="_2-4-2-抽取方式" tabindex="-1"><a class="header-anchor" href="#_2-4-2-抽取方式" aria-hidden="true">#</a> 2.4.2 抽取方式</h3>
<p>将 <code v-pre>Feign</code> 的 <code v-pre>Client</code> 抽取为独立模块，并且把接口有关的 <code v-pre>POJO</code>、默认的 <code v-pre>Feign</code> 配置都放到这个模块中，提供给所有消费者使用。</p>
<p>例如，将 <code v-pre>UserClient</code>、<code v-pre>User</code>、<code v-pre>Feign</code> 的默认配置都抽取到一个 <code v-pre>feign-api</code> 包中，所有微服务引用该依赖包，即可直接使用。</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714214041796.png" alt="image-20210714214041796"></p>
...
<p>首先创建一个 <code v-pre>module</code>，命名为 <code v-pre>feign-api</code>：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714204557771.png" alt="image-20210714204557771"></p>
<p>项目结构：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714204656214.png" alt="image-20210714204656214"></p>
<p>在 <code v-pre>feign-api</code> 中然后引入 <code v-pre>feign</code> 的 <code v-pre>starter</code> 依赖</p>
<div class="language-xml line-numbers-mode" data-ext="xml"><pre v-pre class="language-xml"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>groupId</span><span class="token punctuation">></span></span>org.springframework.cloud<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>groupId</span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>artifactId</span><span class="token punctuation">></span></span>spring-cloud-starter-openfeign<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>artifactId</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后，<code v-pre>order-service</code> 中编写的 <code v-pre>UserClient</code>、<code v-pre>User</code>、<code v-pre>DefaultFeignConfiguration</code> 都复制到 <code v-pre>feign-api</code> 项目中</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714205221970.png" alt="image-20210714205221970"></p>
...
<p>首先，删除 <code v-pre>order-service</code> 中的 <code v-pre>UserClient</code>、<code v-pre>User</code>、<code v-pre>DefaultFeignConfiguration</code> 等类或接口。</p>
<p>在 <code v-pre>order-service</code> 的 <code v-pre>pom</code> 文件中中引入 <code v-pre>feign-api</code> 的依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml"><pre v-pre class="language-xml"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改 <code v-pre>order-service</code> 中的所有与上述三个组件有关的导包部分，改成导入 <code v-pre>feign-api</code> 中的包</p>
<h4 id="_3、重启测试" tabindex="-1"><a class="header-anchor" href="#_3、重启测试" aria-hidden="true">#</a> 3、重启测试</h4>
<p>重启后，发现服务报错了：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714205623048.png" alt="image-20210714205623048"></p>
<p>这是因为 <code v-pre>UserClient</code> 现在在 <code v-pre>com.jmcoding.feign.clients</code> 包下，而 <code v-pre>order-service</code> 的 <code v-pre>@EnableFeignClients</code> 注解是在 <code v-pre>com.jmcoding.order</code> 包下，不在同一个包，无法扫描到 <code v-pre>UserClient</code></p>
<h4 id="_4、解决扫描包问题" tabindex="-1"><a class="header-anchor" href="#_4、解决扫描包问题" aria-hidden="true">#</a> 4、解决扫描包问题</h4>
<p>方式一：指定 <code v-pre>Feign</code> 应该扫描的包（这个包下的所有客户端都会加载）</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>方式二：指定需要加载的 <code v-pre>Client</code> 接口（精准定位，推荐使用这个，不会造成资源浪费）</p>
...
<blockquote>
<p><code v-pre>Spring Cloud Gateway</code> 是 <code v-pre>Spring Cloud</code> 的一个全新项目，该项目是基于 <code v-pre>Spring 5.0</code>，<code v-pre>Spring Boot 2.0</code> 和 <code v-pre>Project Reactor</code> 等响应式编程和事件流技术开发的网关，它旨在为微服务架构提供一种简单有效的统一的 API 路由管理方式</p>
</blockquote>
<h2 id="_3-1-为什么需要网关" tabindex="-1"><a class="header-anchor" href="#_3-1-为什么需要网关" aria-hidden="true">#</a> 3.1 为什么需要网关</h2>
<blockquote>
<p><code v-pre>Gateway</code> 网关是我们服务的守门神，所有微服务的统一入口</p>
</blockquote>
...
<p>架构图：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714210131152.png" alt="image-20210714210131152"></p>
<p><strong>权限控制</strong>：网关作为微服务入口，需要校验用户是是否有请求资格，如果没有则进行拦截。</p>
<p><strong>路由和负载均衡</strong>：一切请求都必须先经过 <code v-pre>gateway</code>，但网关不处理业务，而是根据某种规则，把请求转发到某个微服务，这个过程叫做路由。当然路由的目标服务有多个时，还需要做负载均衡。</p>
<p><strong>限流</strong>：当请求流量过高时，在网关中按照下流的微服务能够接受的速度来放行请求，避免服务压力过大。</p>
<p>在 <code v-pre>SpringCloud</code> 中网关的实现包括两种：</p>
<ul>
<li><code v-pre>gateway</code></li>
<li><code v-pre>zuul</code></li>
</ul>
<p><code v-pre>Zuul</code> 是基于 <code v-pre>Servlet</code> 的实现，属于阻塞式编程。而 <code v-pre>SpringCloudGateway</code> 则是基于 <code v-pre>Spring5</code> 中提供的<code v-pre>WebFlux</code>，属于响应式编程的实现，具备更好的性能。</p>
...
<ol>
<li>创建 <code v-pre>SpringBoot</code> 工程 <code v-pre>gateway</code>，引入网关依赖</li>
<li>编写启动类</li>
<li>编写基础配置和路由规则</li>
<li>启动网关服务进行测试</li>
</ol>
...
<p>创建服务：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714210919458.png" alt="image-20210714210919458"></p>
<p>引入依赖：</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>jmcoding<span class="token punctuation">.</span>gateway</span><span class="token punctuation">;</span>
...
<p>创建 <code v-pre>application.yml</code> 文件，内容如下：</p>
...
<p>本例中，我们将 <code v-pre>/user/**</code>开头的请求，代理到<code v-pre>lb://userservice</code>，<code v-pre>lb</code> 是负载均衡，根据服务名拉取服务列表，实现负载均衡。</p>
<h3 id="_4、重启测试" tabindex="-1"><a class="header-anchor" href="#_4、重启测试" aria-hidden="true">#</a> 4、重启测试</h3>
<p>重启网关，访问<a href="http://localhost:10010/user/1%E6%97%B6%EF%BC%8C%E7%AC%A6%E5%90%88%60/user/**%60%E8%A7%84%E5%88%99%EF%BC%8C%E8%AF%B7%E6%B1%82%E8%BD%AC%E5%8F%91%E5%88%B0uri%EF%BC%9Ahttp://userservice/user/1%EF%BC%8C%E5%BE%97%E5%88%B0%E4%BA%86%E7%BB%93%E6%9E%9C%EF%BC%9A" target="_blank" rel="noopener noreferrer">http://localhost:10010/user/1时，符合`/user/**`规则，请求转发到uri：http://userservice/user/1，得到了结果：<ExternalLinkIcon/></a></p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714211908341.png" alt="image-20210714211908341"></p>
<h3 id="_5、网关路由的流程图" tabindex="-1"><a class="header-anchor" href="#_5、网关路由的流程图" aria-hidden="true">#</a> 5、网关路由的流程图</h3>
<p>整个访问的流程如下：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714211742956.png" alt="image-20210714211742956"></p>
<h3 id="_6、总结" tabindex="-1"><a class="header-anchor" href="#_6、总结" aria-hidden="true">#</a> 6、总结</h3>
<p>网关搭建步骤：</p>
<ol>
<li>
<p>创建项目，引入 <code v-pre>nacos</code> 服务发现和 <code v-pre>gateway</code> 依赖</p>
</li>
<li>
<p>配置 <code v-pre>application.yml</code>，包括服务基本信息、<code v-pre>nacos</code> 地址、路由</p>
</li>
</ol>
<p>路由配置包括：</p>
<ol>
...
<li>
<p>路由目标（<code v-pre>uri</code>）：路由的目标地址，<code v-pre>http</code> 代表固定地址，<code v-pre>lb</code> 代表根据服务名负载均衡</p>
</li>
<li>
<p>路由断言（<code v-pre>predicates</code>）：判断路由的规则，</p>
</li>
<li>
<p>路由过滤器（<code v-pre>filters</code>）：对请求或响应做处理</p>
</li>
</ol>
<blockquote>
...
<p>我们在配置文件中写的断言规则只是字符串，这些字符串会被 <code v-pre>Predicate Factory</code> 读取并处理，转变为路由判断的条件</p>
<p>例如 <code v-pre>Path=/user/**</code> 是按照路径匹配，这个规则是由</p>
<p><code v-pre>org.springframework.cloud.gateway.handler.predicate.PathRoutePredicateFactory</code>类来</p>
<p>处理的，像这样的断言工厂在 <code v-pre>SpringCloudGateway</code> 还有十几个：</p>
<table>
<thead>
<tr>
...
<blockquote>
<p>我们只需要掌握 <code v-pre>Path</code> 这种路由工程就可以了</p>
</blockquote>
<h2 id="_3-4-过滤器工厂" tabindex="-1"><a class="header-anchor" href="#_3-4-过滤器工厂" aria-hidden="true">#</a> 3.4 过滤器工厂</h2>
<p><code v-pre>GatewayFilter</code> 是网关中提供的一种过滤器，可以对进入网关的请求和微服务返回的响应做处理：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714212312871.png" alt="image-20210714212312871"></p>
<h3 id="_3-4-1-路由过滤器的种类" tabindex="-1"><a class="header-anchor" href="#_3-4-1-路由过滤器的种类" aria-hidden="true">#</a> 3.4.1 路由过滤器的种类</h3>
<p><code v-pre>Spring</code> 提供了31种不同的路由过滤器工厂。例如：</p>
<table>
<thead>
<tr>
<th><strong>名称</strong></th>
...
<p>下面我们以 <code v-pre>AddRequestHeader</code> 为例来讲解。</p>
<blockquote>
<p><strong>需求</strong>：给所有进入 <code v-pre>userservice</code> 的请求添加一个请求头：<code v-pre>Truth=jmcoding is freaking awesome!</code></p>
</blockquote>
<p>只需要修改 <code v-pre>gateway</code> 服务的 <code v-pre>application.yml</code> 文件，添加路由过滤即可：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  <span class="token key atrule">cloud</span><span class="token punctuation">:</span>
    <span class="token key atrule">gateway</span><span class="token punctuation">:</span>
...
<p>当前过滤器写在 <code v-pre>userservice</code> 路由下，因此仅仅对访问 <code v-pre>userservice</code> 的请求有效</p>
</blockquote>
<h3 id="_3-4-3-默认过滤器" tabindex="-1"><a class="header-anchor" href="#_3-4-3-默认过滤器" aria-hidden="true">#</a> 3.4.3 默认过滤器</h3>
<p>如果要对所有的路由都生效，则可以将过滤器工厂写到 <code v-pre>default</code> 下。格式如下：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  <span class="token key atrule">cloud</span><span class="token punctuation">:</span>
    <span class="token key atrule">gateway</span><span class="token punctuation">:</span>
      <span class="token key atrule">routes</span><span class="token punctuation">:</span>
...
<p><code v-pre>defaultFilters</code> 的作用是什么？</p>
<p>① 对所有路由都生效的过滤器</p>
...
<p>全局过滤器的作用也是处理一切进入网关的请求和微服务响应，与 <code v-pre>GatewayFilter</code> 的作用一样。区别在于 <code v-pre>GatewayFilter</code> 通过配置定义，处理逻辑是固定的；而 <code v-pre>GlobalFilter</code> 的逻辑需要自己写代码实现。</p>
<p>定义方式是实现 <code v-pre>GlobalFilter</code> 接口。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">GlobalFilter</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 <code v-pre>filter</code> 中编写自定义逻辑，可以实现下列功能：</p>
<ul>
<li>登录状态判断</li>
<li>权限校验</li>
<li>请求限流等</li>
...
<ul>
<li>
<p>参数中是否有 <code v-pre>authorization</code></p>
</li>
<li>
<p><code v-pre>authorization</code>参数值是否为 <code v-pre>admin</code></p>
</li>
</ul>
<p>如果同时满足则放行，否则拦截</p>
<p>实现：</p>
<p>在 <code v-pre>gateway</code> 中定义一个过滤器：</p>
...
<p>请求进入网关会碰到三类过滤器：当前路由的过滤器、<code v-pre>DefaultFilter</code>、<code v-pre>GlobalFilter</code></p>
<p>请求路由后，会将当前路由过滤器和 <code v-pre>DefaultFilter</code> 、<code v-pre>GlobalFilter</code>，合并到一个过滤器链（集合）中，排序后依次执行每个过滤器：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714214228409.png" alt="image-20210714214228409"></p>
<p>排序的规则是什么呢？</p>
<ul>
<li>每一个过滤器都必须指定一个 <code v-pre>int</code> 类型的 <code v-pre>order</code> 值，<strong><code v-pre>order</code> 值越小，优先级越高，执行顺序越靠前</strong>。</li>
<li><code v-pre>GlobalFilter</code> 通过实现 <code v-pre>Ordered</code> 接口，或者添加 <code v-pre>@Order</code> 注解来指定 <code v-pre>order</code> 值，由我们自己指定</li>
<li>路由过滤器和 <code v-pre>defaultFilter</code> 的 <code v-pre>order</code> 由 <code v-pre>Spring</code> 指定，默认是按照声明顺序从1递增。</li>
<li>当过滤器的 <code v-pre>order</code> 值一样时，会按照 <code v-pre>defaultFilter</code> &gt; 路由过滤器 &gt; <code v-pre>GlobalFilter</code> 的顺序执行。</li>
</ul>
<p>详细内容，可以查看源码：</p>
...
<ul>
<li>
<p>域名不同： <a href="http://www.taobao.com" target="_blank" rel="noopener noreferrer">www.taobao.com<ExternalLinkIcon/></a> 和 <a href="http://www.taobao.org" target="_blank" rel="noopener noreferrer">www.taobao.org<ExternalLinkIcon/></a> 和 <a href="http://www.jd.com" target="_blank" rel="noopener noreferrer">www.jd.com<ExternalLinkIcon/></a> 和 <a href="http://miaosha.jd.com" target="_blank" rel="noopener noreferrer">miaosha.jd.com<ExternalLinkIcon/></a></p>
</li>
<li>
<p>域名相同，端口不同：<code v-pre>localhost:8080</code> 和 <code v-pre>localhost:8081</code></p>
</li>
</ul>
<p>跨域问题：浏览器禁止请求的发起者与服务端发生跨域 <code v-pre>ajax</code> 请求，请求被浏览器拦截的问题</p>
<p>解决方案：<code v-pre>CORS</code>，这个以前应该学习过，这里不再赘述了。不知道的小伙伴可以查看<a href="https://www.ruanyifeng.com/blog/2016/04/cors.html" target="_blank" rel="noopener noreferrer">https://www.ruanyifeng.com/blog/2016/04/cors.html<ExternalLinkIcon/></a></p>
<h3 id="_3-6-2-模拟跨域问题" tabindex="-1"><a class="header-anchor" href="#_3-6-2-模拟跨域问题" aria-hidden="true">#</a> 3.6.2 模拟跨域问题</h3>
<p>找到课前资料的页面文件：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714215713563.png" alt="image-20210714215713563"></p>
<p>放入 <code v-pre>tomcat</code> 或者 <code v-pre>nginx</code> 这样的 <code v-pre>web</code> 服务器中，启动并访问。</p>
<p>可以在浏览器控制台看到下面的错误：</p>
<p><img src="https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/1273/image-20210714215832675.png" alt="image-20210714215832675"></p>
<p>从 <code v-pre>localhost:8090</code> 访问 <code v-pre>localhost:10010</code>，端口不同，显然是跨域的请求。</p>
<h3 id="_3-6-3-解决跨域问题" tabindex="-1"><a class="header-anchor" href="#_3-6-3-解决跨域问题" aria-hidden="true">#</a> 3.6.3 解决跨域问题</h3>
<p>在 <code v-pre>gateway</code> 服务的 <code v-pre>application.yml</code> 文件中，添加下面的配置：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  <span class="token key atrule">cloud</span><span class="token punctuation">:</span>
    <span class="token key atrule">gateway</span><span class="token punctuation">:</span>
